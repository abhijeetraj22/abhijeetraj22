  ...
  if (window.particlesJS) {
    particlesJS('particles-js', {
      "particles": {
        ...
        "line_linked": { "enable": true, "distance": 120, "color": "#00e6ff", "opacity": 0.04, "width": 1 },
        "move": { "enable": true, "speed": 1.2, "direction": "none", "out_mode": "bounce" }
      },
      "interactivity": {
        "detect_on": "canvas",
        "events": { "onhover": { "enable": true, "mode": "grab" }, "onclick": { "enable": true, "mode": "push" } },
        "modes": { "grab": { "distance": 160, "line_linked": { "opacity": 0.15 } }, "push": { "particles_nb": 3 } }
      },
      "retina_detect": true
    });
  }
...
(function(){
  ...
  contactForm.addEventListener('submit', async function (ev) {
    ...
    if (name.trim() === '') { nameErr.textContent = 'Name is required.'; valid = false; }
    if (!email.match(emailPattern)) { emailErr.textContent = 'Enter a valid email.'; valid = false; }
    if (message.trim() === '') { msgErr.textContent = 'Message cannot be empty.'; valid = false; }
    if (!valid) return;
...
rightBtn.addEventListener("click", () => { moveRight(); restartAuto(); });
leftBtn.addEventListener("click", () => { moveLeft(); restartAuto(); });

function startAuto() { autoSlide = setInterval(moveRight, 2000); }
