# <p align="center">👋 Hi, I'm <strong>Abhijeet Raj</strong></p>

<p align="center">
  <strong>Azure | Data Engineering | Python Developer | ETL | ADF | SQL | Power BI | DevOps</strong>  
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Azure-Data%20Engineer-blue?logo=microsoftazure" />
  <img src="https://img.shields.io/badge/Python-Developer-yellow?logo=python" />
  <img src="https://img.shields.io/badge/ETL-Pipelines-green" />
  <img src="https://img.shields.io/badge/PowerBI-Analyst-F2C811?logo=powerbi" />
  <img src="https://img.shields.io/badge/SQL-Expert-blue?logo=postgresql" />
</p>

---

# 🌟 About Me

* 🔹 Building **Azure Data Engineering** solutions (ADF, Databricks, Data Lake, Synapse)
* 🔹 Experience with **real-world automation** in Python
* 🔹 Strong in **Python Automation**, ETL pipelines, file processing & analytics
* 🔹 Comfort with **ETL Pipelines, SQL development, dashboards & data analytics**
* 🔹 Experience with **automation systems** for data processing (CBSE results, attendance, PDF pipelines)
* 🔹 Practicing **DevOps CI/CD** with Azure DevOps & GitHub Actions
* 🔹 Passionate about **clean code, automation, and scalable data systems**

---

# 🚀 Tech Stack

### 🧑‍💻 Languages & Frameworks

`Python` • `SQL` • `PySpark` • `JavaScript` • `PowerShell`

### ☁ Cloud & Big Data

`Azure Data Factory` • `Azure Databricks` • `Data Lake` • `Azure Storage`
`Azure Synapse` • `Azure Functions`

### 🗄 Databases

`SQL Server` • `MySQL` • `SQLite` • `PostgreSQL`

### 📊 Analytics

`Power BI` • `Tableau` • `Matplotlib` • `Seaborn`

### ♾ DevOps

`Azure DevOps` • `GitHub Actions` • `CI/CD Pipelines`

---

# 🏆 Featured Projects
Below are the **top projects selected from my GitHub repositories**, each analyzed and summarized with direct links.

---
# 🧩 Azure Projects
## 📌 Azure Data Factory ETL Pipeline

🔗 Repo: [https://github.com/abhijeetraj22/AzureDataFactortETL-Project](https://github.com/abhijeetraj22/AzureDataFactortETL-Project)  
🛠 Azure Data Factory • SQL • Data Lake • ETL

* Automated data ingestion & transformation
* Parameterized pipelines & datasets
* Validation, scheduling & logging
* End-to-end ETL pipeline
* Source → Raw → Transform → SQL Sink
* Data mapping + dynamic linked services
* Orchestration with ADF pipelines & triggers

---

## ⚡ Azure Data Pipeline – ADF + Databricks + Synapse

🔗 Repo: [https://github.com/abhijeetraj22/Azure-Data-Pipeline-with-ADF-Databricks-Synapse](https://github.com/abhijeetraj22/Azure-Data-Pipeline-with-ADF-Databricks-Synapse)  
🛠 ADF • Databricks • Synapse • Data Lake

* Medallion architecture (Bronze → Silver → Gold)
* Databricks PySpark transformations
* SQL analytics using Synapse
* Cloud-based scalable ETL

---

## 🤖 Microsoft Learn AI Skills Challenge

🔗 Repo: [https://github.com/abhijeetraj22/Microsoft-Learn-AI-Skills-Challenge](https://github.com/abhijeetraj22/Microsoft-Learn-AI-Skills-Challenge)  
🛠 Azure AI Studio • Cognitive Services

* NLP & Computer Vision demos
* OCR & Data extraction
* AI-900 style practical implementation

---

# 🧩 Automation & Python Projects

## 🧾 PDF Processing Automation – Ballard Spahr

🔗 Repo: [https://github.com/abhijeetraj22/PDF-Processing-Automation-Ballard-Spahr-](https://github.com/abhijeetraj22/PDF-Processing-Automation-Ballard-Spahr-)  
🛠 Python • SQL Server • PDF Libraries

* Automated pipeline for merging, renaming & organizing PDFs
* SQL-based tracking + error logging
* End-to-end workflow with Windows Task Scheduler
* Real corporate-style automation project

---

## 🏫 OMK CBSE Result Converter – Python GUI (Tkinter)

🔗 Repo: [https://github.com/abhijeetraj22/OMK_CBSE_RESULT](https://github.com/abhijeetraj22/OMK_CBSE_RESULT)  
🛠 Python • Tkinter • Pandas • Excel Automation

* Convert CBSE raw TXT → formatted Excel
* Automatic extraction of student details
* Multi-sheet support
* User-friendly GUI created with Tkinter

---

## ⏱️ OMK Attendance GUI – Excel Attendance Automation

🔗 Repo: [https://github.com/abhijeetraj22/OMK_ATTENDANCE_GUI](https://github.com/abhijeetraj22/OMK_ATTENDANCE_GUI)  
🛠 Python • Tkinter • Pandas

* Admin + Teacher modules
* Grace time logic (e.g., 8:15 → 8:25 AM)
* Sheet dropdown selectors
* Process & export attendance reports

---

## 📤 SSIS - Excel/CSV to SQL Import Automation

🔗 Repo: [https://github.com/abhijeetraj22/Excel-CSV-to-SQL-Import-Automation](https://github.com/abhijeetraj22/Excel-CSV-to-SQL-Import-Automation)  
🛠 Python • SQL Server Agent

* Automated Excel/CSV → SQL loading
* Auto-schema creation & column type detection
* Data cleansing + mapping
* Bulk insert logic

---

## 🧪 Quiz System – Python Application

🔗 Repo: [https://github.com/abhijeetraj22/Quiz_Systems](https://github.com/abhijeetraj22/Quiz_Systems)  
🛠 Python • File & DB Storage

* Console-based quiz engine
* Stores results per user
* Easily expandable question bank

---

## 🌍 India & World COVID-19 Data Analytics

🔗 Repo: [https://github.com/abhijeetraj22/India-World-COVID-19-Anlyz-Visualiz-Predict](https://github.com/abhijeetraj22/India-World-COVID-19-Anlyz-Visualiz-Predict)  
🛠 Python • Pandas • Matplotlib

* Multi-country trend comparisons
* COVID-19 data exploration
* Visualizations (daily, cumulative)
* Prediction models
* Time-series charts & heatmaps

---

## 📘 Facebook Page Data Analyzer

🔗 Repo: [https://github.com/abhijeetraj22/FB_Page-Decoder-Data_Analysis](https://github.com/abhijeetraj22/FB_Page-Decoder-Data_Analysis)  
🛠 Python • Web Scraping • Pandas

* Extracts Facebook page metrics
* Engagement pattern insights
* Automated data parsing
* Social media page scraper
* Follower growth patterns

---

## 🔬 Research – COVID 19 India & Iran

🔗 Repo: [https://github.com/abhijeetraj22/Research-about-covid19-in-India-and-Iran2020](https://github.com/abhijeetraj22/Research-about-covid19-in-India-and-Iran2020)  
🛠 Python • Data Science

* Side-by-side statistical comparison
* Comparative analysis between 2 countries
* Trendline visualization
* Recovery & fatality projections
* Graph-based insights

---

## 📱 TSF GRIP – IoT & Device Data Projects

🔗 Repo: [https://github.com/abhijeetraj22/TSF-GRIP_IOT_Tasks](https://github.com/abhijeetraj22/TSF-GRIP_IOT_Tasks)  
🛠 IoT Concepts • Python

* Real-time sensor logical tasks
* Data aggregation examples
* Demonstrates embedded IoT logic

---

## 🎓 TSF GRIP – Machine Learning Tasks

🔗 Repo: [https://github.com/abhijeetraj22/TSF-GRIP_Tasks](https://github.com/abhijeetraj22/TSF-GRIP_Tasks)  
🛠 Python • ML Basics

* Feature engineering
* Prediction model exercises
* Exploratory analytics reports

---

## 👁 OpenCV Computer Vision

🔗 Repo: [https://github.com/abhijeetraj22/ShapeAI_ComputerVision_with_OpenCV](https://github.com/abhijeetraj22/ShapeAI_ComputerVision_with_OpenCV)  
🛠 OpenCV • Image Processing

* Edge detection
* Object recognition basics
* Real-time webcam interactive tasks

---

## 🤖 ShapeAI Deep Learning

🔗 Repo: [https://github.com/abhijeetraj22/ShapeAI_DL](https://github.com/abhijeetraj22/ShapeAI_DL)  
🛠 Keras • DNN

* Intro deep learning models
* Classification training
* Accuracy tracking & tuning

---

## 📊 Power BI + Tableau Analytics Project

🔗 Repo: [https://github.com/abhijeetraj22/Jobaaj-Data-Tableau-PowerBI-workshop](https://github.com/abhijeetraj22/Jobaaj-Data-Tableau-PowerBI-workshop)  
🛠 Power BI • Tableau • DAX

* Business dashboards
* Trend and KPI analysis
* Sales analytics

---

## 🔐 Cybersecurity Demonstration Project

🔗 Repo: [https://github.com/abhijeetraj22/ShapeAI_CyberSecurity_Project](https://github.com/abhijeetraj22/ShapeAI_CyberSecurity_Project)  
🛠 Cybersec • Python

* Demonstrates password strength checks
* Login validation flow
* Basic authentication system logic

---

# 🔥 GitHub Stats

<p align="center">
  <img height="180" src="https://github-readme-streak-stats.herokuapp.com/?user=abhijeetraj22" />
  <img height="180" src="https://github-readme-stats.vercel.app/api?username=abhijeetraj22&show_icons=true" />
</p>
<p align="center">
  <img height="180" src="https://github-readme-stats-fast.vercel.app/api/top-langs?username=abhijeetraj22&layout=compact&theme=dark" />
</p>
<p align="center">
  <img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=abhijeetraj22&layout=compact" />
</p>

# 🏅 Achievements

* Built real-world data & automation projects
* Completed **Microsoft AI Skills Challenge**
* Multiple real datasets analyzed
* End-to-end Azure ETL pipelines deployed
* Multiple AI & analytics research projects
* Strong portfolio in **Python,  Azure, Data Engineering & Analytics**
* Completed multiple ShapeAI Bootcamps (DL, CV, Cybersecurity)

# 📬 Connect

📧 Email: **[abhijeetraj22@gmail.com](mailto:abhijeetraj22@gmail.com)**
<p align="left"> <a href="https://github.com/abhijeetraj22" target="_blank"> <img src="https://img.shields.io/badge/GitHub-%2312100E.svg?style=for-the-badge&logo=github&logoColor=white" /> </a> <a href="https://www.linkedin.com/in/rajabhijeet22" target="_blank"> <img src="https://img.shields.io/badge/LinkedIn-%230A66C2.svg?style=for-the-badge&logo=linkedin&logoColor=white" /> </a> </p>

---

# 👨‍💻 Looking For

💼 **Azure Data Engineer / Data Analyst / Python Developer / ETL Engineer / SQL Developer / Automation Engineer roles**   
📌 Open to **Full-time | Internship | Hybrid | Remote**

